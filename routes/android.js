...
module.exports.add_save = function(req, res) {
    ...
    req.getConnection(function(err, connection) {
        ...
        if ( typeof input.name === 'undefined') 
        {
            ...
            var query = connection.query("SELECT name FROM android WHERE name = ? ",input.name,function(err,data){
                if(err){
                    console.log(err2);
                    console.log('err2');
                    ...
                    var query = connection.query("INSERT into android set ?", data, function(err, rows) {
                        if (err) {
                            console.log(err);
                          console.log('err3');
